...
_uptime_info() {
    awk '{printf("%dd %02d:%02d",($1/60/60/24),($1/60/60%24),($1/60%60))}' /proc/uptime
}

#
# The below configurations are inspired by Gregory Pakosz (@gpakosz)
...
_apply_theme() {
  ...
  tmux_conf_theme_terminal_title=$(echo "$tmux_conf_theme_terminal_title" | sed \
    -e "s%#{circled_window_index}%#($SCRIPT_DIR/tmux_run_shell.sh _circled #I)%g" \
    -e "s%#{circled_session_name}%#($SCRIPT_DIR/tmux_run_shell.sh _circled #S)%g" \
    -e "s%#{username}%#($SCRIPT_DIR/tmux_run_shell.sh _username #{pane_tty} false #D)%g" \
    -e "s%#{hostname}%#($SCRIPT_DIR/tmux_run_shell.sh _hostname #{pane_tty} false #D)%g" \
    -e "s%#{username_ssh}%#($SCRIPT_DIR/tmux_run_shell.sh _username #{pane_tty} true #D)%g" \
    -e "s%#{hostname_ssh}%#($SCRIPT_DIR/tmux_run_shell.sh _hostname #{pane_tty} true #D)%g")
  ...
  tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | sed \
    -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]#[$tmux_conf_theme_root_attr]#($SCRIPT_DIR/tmux_run_shell.sh _root #{pane_tty} #D)#[inherit]%g")
...
EOF
  ...
  case "$status_left $status_right" in
    *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
      ...
      status_right="#($SCRIPT_DIR/tmux_run_shell.sh _battery)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*|*'#{uptime_s}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        ...
        -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
        -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
      status_right="#($SCRIPT_DIR/tmux_run_shell.sh _uptime)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{loadavg}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
      status_right="#($SCRIPT_DIR/tmux_run_shell.sh _loadavg)$status_right"
      ;;
  esac

  status_left=$(echo "$status_left" | sed "s%#{circled_session_name}%#($SCRIPT_DIR/tmux_run_shell.sh _circled #S)%g")
  status_right=$(echo "$status_right" | sed "s%#{circled_session_name}%#($SCRIPT_DIR/tmux_run_shell.sh _circled #S)%g")
